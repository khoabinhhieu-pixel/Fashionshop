...
const productObjectSchema = z.object({
  ...
  stock: z.coerce.number().int().min(0, "Số lượng tồn kho không hợp lệ"),
  sizes: z.string().trim().optional().default(""),
  description: z.string().trim().min(1, "Vui lòng nhập mô tả"),
  badge: z.enum(["NONE", "MOI", "BAN_CHAY", "SALE", "HOT"]).default("NONE"),
});
...
function fieldsFromFormData(formData: FormData) {
  ...
  return {
    ...
    discountPercent: discountPercentRaw || undefined,
...
function extractNewFiles(formData: FormData): File[] {
  // Một <input type="file" multiple> không chọn gì vẫn sinh 1 File rỗng
  // 0-byte trong FormData khi submit — phải lọc bỏ trước khi validate.
  return formData
    .getAll("images")
  ...
  try {
    await prisma.product.create({
      data: {
        ...
        originalPrice: computeOriginalPrice(parsed.data.price, parsed.data.discountPercent),
...
const updateSchema = productObjectSchema.omit({ id: true });
