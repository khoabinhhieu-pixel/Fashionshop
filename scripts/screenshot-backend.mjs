...
async function shoot(page, name) {
  // Every section on this site fades in via Framer Motion's `whileInView`
  // (see components/ui/Reveal.tsx). A bare fullPage screenshot without
  // scrolling first captures unvisited sections as solid black (stuck at
  // opacity: 0) — scroll through so every IntersectionObserver fires first.
...
try {
  // ---- 1. Register a brand-new account end to end ----
  {
  ...
  // ---- 3. Demo customer session: PDP, review, wallet, checkout w/ wallet ----
  {
    const context = await browser.newContext({ viewport: { width: 1440, height: 900 } });
    const page = await context.newPage();
    await loginAs(page, "demo@fashionshop.vn", "Demo123456");

    // Header shows logged-in state (account icon -> /account)
    ...
    // A generous top-up so the later wallet-checkout step always has
    // enough balance, regardless of how much prior QA runs already spent.
    ...
    // Add a known, stable product to cart (New Arrivals sorts newest-first,
    // so "first card on the homepage" is unstable once QA creates products
    // via the admin flow below — target a fixed product by slug instead).
    await page.goto(`${base}/products/wool-overcoat`, { waitUntil: "networkidle" });
  ...
  // ---- 4. Admin session: products list, create ----
  {
    const context = await browser.newContext({ viewport: { width: 1440, height: 900 } });
    const page = await context.newPage();
