...
import { PRODUCT_BADGE_LABEL } from "@/lib/constants";
...
const BADGES = Object.entries(PRODUCT_BADGE_LABEL).map(([value, label]) => ({
  value,
  label,
}));

function currentDiscountPercent(product?: Product): number | undefined {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <div className="grid gap-5 sm:grid-cols-4">
        ...
        />
        <Field
          label="Giảm giá (%, để trống nếu không giảm giá)"
          type="number"
          name="discountPercent"
          min={0}
          max={99}
          defaultValue={currentDiscountPercent(product)}
          placeholder="20"
        />
        <Field
          label="Danh mục"
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        ...
        <label className="flex flex-col gap-2">
          ...
          >
            {BADGES.map((b) => (
              <option key={b.value} value={b.value} className="bg-bg text-fg">
                {b.label}
              </option>
            ))}
          </select>
      ...
      <Field
        label="Kích cỡ (cách nhau bởi dấu phẩy, để trống nếu không phân loại size)"
        name="sizes"
